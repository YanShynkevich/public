...
function syncQueryAromatics1(molBlock: string,  bonds2Change: Array<number> | null = null) : string | Array<number> {
  let curPos = 0;
  ...
  //console.log('bondCounts: ' + bondCounts);
...
for (let bondRowI = 0; bondRowI < bondCounts; bondRowI++) {
  curPos = molBlock.indexOf('\n', curPos) + 1;
  //console.log('curPos: ' + curPos + " read: " + read);
  if (read) {
    //console.log('before bondOrder: ' + bondOrder);
    bondOrder = parseInt(molBlock.substring(curPos + 8, curPos + 9));
    // console.log('bondOrder: ' + bondOrder + " str " +  molBlock);
    if (bondOrder === 4)
      bonds2Change.push(bondRowI);
  }
  else {
    if (bonds2Change.includes(bondRowI)) {
      console.log("Reason for a change " + bondRowI);
      molBlock = molBlock.slice(0, curPos + 8) + '4' + molBlock.slice(curPos + 9);
    }
  }
}

//console.log("Array:"  + bonds2Change.length);
return read ? bonds2Change : molBlock;
}

function syncQueryAromatics2(molBlockAroma: string, molBlock : string) : string {
  const bonds2Change = syncQueryAromatics1(molBlock);
  //console.log("Array: "  + bonds2Change.length);
  const molModified = syncQueryAromatics1(molBlockAroma, bonds2Change as Array<number>);
  return molModified as string;
}
/*
function syncQueryAromatics(molBlockAroma: string, molBlock : string) : string {
  let curPos = 0;
  curPos = molBlock.indexOf('\n', curPos) + 1;
  curPos = molBlock.indexOf('\n', curPos) + 1;
  curPos = molBlock.indexOf('\n', curPos) + 1;
      ...
      try {
        if (isMolBlock(queryMolString)) {
          ...
          if (queryMolString.includes(' H '))
            ...
            console.log('queryMolString');
            console.log(queryMolString);

            //console.log('Aroma Mol');
            //console.log(molBlockAroma);
            const newQueryMolString = syncQueryAromatics2(molBlockAroma, queryMolString);
            console.log('After Modify');
            console.log(newQueryMolString);
