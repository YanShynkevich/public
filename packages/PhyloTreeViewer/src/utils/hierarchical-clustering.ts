...
import {TAGS as thTAGS} from './tree-helper';
...
import {Newick, NodeType} from '@datagrok-libraries/bio';
...
export async function hierarchicalClusteringUI(
  ...
  const newickRoot: NodeType = Newick.parse_newick(newickStr);
  ...
  injectTreeForGridUI2(tv.grid, newickRoot, df, clusterDf, null, 200);
}
...
export async function hierarchicalClusteringExec(
  ...
  let newick = '';
  try {
    newick = await grok.functions.call('PhyloTreeViewer:getNewick', {
      data: preparedDf,
    ...
    const errStr = errorToConsole(err);
    console.error('PhyloTreeViewer: hierarchicalClusteringExec() ' + `${errStr}`);
    throw err;
  }
