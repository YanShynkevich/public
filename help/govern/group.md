...
Datagrok supports granting membership via URL. You can invite unlimited users outside the Datagrok instance to join a particular group. You can also apply this functionality to track a cohort of users who access the platform via a specific link, for example, after watching a demo.

### Create an invitation link

1. Go to **Manage** > **Groups**.
1. Right-click the group and select **Edit…**. A dialog opens.
1. To generate a password, click the button next to the **Password** field. The dialog displays an autogenerated _password_.
1. Copy the _password_ and use it to create an invitation link as follows:

   `<_instance URL_>?groupPassword=<_password_>`
...
![Providing membership by link](group-membership-via-url.gif)
